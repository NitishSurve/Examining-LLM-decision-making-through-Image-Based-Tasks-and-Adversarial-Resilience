...
#                 print("nitish bhdwa h 1")
#                 # save_path = output_path + 'model-' + str(e)
#                 import os

#                 save_path = os.path.join(output_path, 'model-' + str(e))
                

#                 print("nitish bhdwa h 1")
#                 os.makedirs(os.path.dirname(save_path), exist_ok=True)  # ensure dirs exist
#                 print('chmadi')
#                 DLogger.logger().debug('Trained model saved to: ' + save_path)
#                 print("nitish bhdwa h 1")
...
class DQNAgent:
    def __init__(self, state_size, action_size, buffer_size, epsilon, lr):
        q_size = buffer_size
        self.state_size = state_size
        self.action_size = action_size
        self.memory = {
            'state': ArrQ(q_size, [state_size]),
            'action': ArrQ(q_size, [action_size]),
            'reward': ArrQ(q_size, [1]),
            'next_state': ArrQ(q_size, [state_size]),
            'done': ArrQ(q_size, [1])
    ...
    def train(self, env, batch_size=50, total_episodes=200000, output_path=None):
        ...
        for e in range(total_episodes):
            if e % 2000 == 0:
                ...
                save_path = os.path.join(output_path, f'model-{e}.keras')
                os.makedirs(os.path.dirname(save_path), exist_ok=True)
                print(f'dir crearted here -- {save_path}')
                DLogger.logger().debug('Trained model saved to: ' + save_path)
                self.model.save(save_path)
            ...
            if done[0]:
                ...
                DLogger.logger().debug(
                    "episode: {}/{}, score: {}, e: {:.2}, reward: {}, seudo-rew:{} max: {} init-act: {}"
                    .format(e, total_episodes, 0, self.epsilon, total_r.mean(), total_psudo_r.mean(),
                            self.next_max, self.model(state).numpy().mean(axis=0))
                )
                total_r = 0
